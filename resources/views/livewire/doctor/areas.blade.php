<div class="spec_register1_left_block">
    ...
    <div class="_flex-display _align-center spec_register1_addresses">
        <div class="_flex-display _align-center spec_register1_line spec_register1_address">
            <span>{{ $area ?: 'Район/Метро' }}</span>
            <button wire:click="resetArea">
    ...
    <button wire:click="$set('showAreaModal', true)" class="btn white_rose_btn add_btn add_area">
    ...
    @if ($showAreaModal)
    <div wire:key="area-modal" id="add_area" class="_flex-display _justify-content-center _align-center screen">
        <div class="window add_info_window">
            <div class="_flex-display _justify-content-between _align-center window_top">
                <h4>{{__('Район/Метро')}}</h4>
                <button wire:click="$set('showAreaModal', false)" id="window_close" class="window_close">
            ...
            <div class="search_field search_field_input">
                <input
                    ...
                    class="add_area"
                    type="text"
                    placeholder="{{__('Введіть район/метро..')}}"
                    wire:model.live="search"
                />
            </div>
            <div class="_flex-display _align-center area_metro_tabs">
                <div wire:click="setAreaStep(1)" class="_flex-display _align-center {{ $areaStep === 1 ? 'rose_btn' : 'white_rose_btn' }} area_metro_tab">{{__('Район')}}</div>
                <div wire:click="setAreaStep(2)" class="_flex-display _align-center {{ $areaStep === 2 ? 'rose_btn' : 'white_rose_btn' }} area_metro_tab">{{__('Метро')}}</div>
            </div>
            @if($areaStep === 1)
                <div id="area_block">
                    <ul>
                        @forelse ($filteredAreas as $item)
                            <li wire:click="selectArea('{{ $item }}')"
            ...
            @elseif($areaStep === 2)
                <div id="metro_block">
                    <ul>
                        @forelse ($filteredMetros as $item)
                            <li wire:click="selectArea('{{ $item }}')"
                                class="px-3 py-1 hover:bg-blue-100 cursor-pointer">
                                {{ $item }}
                            </li>
