<div class="spec_register_wrapper">
    <div class="spec_register_wrapper">
        <h5>{{__('Ваші Послуги')}}</h5>
        <div class="_flex-display _align-center spec_register_buttons">
            <button wire:click="openAddModal" class="_flex-display _justify-content-center _align-center btn white_rose_btn add_service">
                ...
                </svg> {{__('Додати послугу')}}
            </button>
            <button wire:click="openCreateModal" class="_flex-display _justify-content-center _align-center btn white_rose_btn add_extra_service">
                <svg viewBox="0 0 14 14" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="14.000000" height="14.000000" fill="none">
                    <rect id="ic:round-plus" width="14.000000" height="14.000000" x="0.000000" y="0.000000" fill="rgb(255,255,255)" fill-opacity="0" />
                    <path id="Vector" d="M13 8L8 8L8 13C8 13.2652 7.89464 13.5196 7.70711 13.7071C7.51957 13.8946 7.26522 14 7 14C6.73478 14 6.48043 13.8946 6.29289 13.7071C6.10536 13.5196 6 13.2652 6 13L6 8L1 8C0.734784 8 0.48043 7.89464 0.292893 7.70711C0.105357 7.51957 0 7.26522 0 7C0 6.73478 0.105357 6.48043 0.292893 6.29289C0.48043 6.10536 0.734784 6 1 6L6 6L6 1C6 0.734784 6.10536 0.480429 6.29289 0.292893C6.48043 0.105357 6.73478 -8.88178e-16 7 0C7.26522 -8.88178e-16 7.51957 0.105357 7.70711 0.292893C7.89464 0.480429 8 0.734784 8 1L8 6L13 6C13.2652 6 13.5196 6.10536 13.7071 6.29289C13.8946 6.48043 14 6.73478 14 7C14 7.26522 13.8946 7.51957 13.7071 7.70711C13.5196 7.89464 13.2652 8 13 8Z" fill="rgb(243,150,162)" fill-rule="nonzero" />
                </svg> {{__('Створити власну полугу')}}
            </button>
        </div>
        <div class="_flex-display _justify-content-between add_doctor_prices_list">
            @foreach($services as $service)
                <div class="_flex-display _justify-content-between doctor_right_price_line">
                    <div class="_flex-display _justify-content-between doctor_right_price">
                        <span>{{ $service['name'] }}</span>
                        <span>@if($service['pivot']['prefix'] === 'for')
                                {{ __('від') }}
                            @else
                                {{ __('') }}
                            @endif
                            {{ number_format($service['pivot']['price'], 0, ',', ' ') }}₴</span></div>
                    <div class="_flex-display _justify-content-between _align-center doctor_right_edit">
                        <button wire:click="openEditModal({{ $service['id'] }})" class="_flex-display btn white_rose_btn price_edit_btn">{{__('Редагувати')}}</button>
                        <button wire:click="deleteService({{ $service['id'] }})" class="close_btn">
                            <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24.000000" height="24.000000" fill="none" clip-path="url(#clipPath_2)" customFrame="url(#clipPath_2)">
                                ...
                                <g id="material-symbols:close-rounded" clip-path="url(#clipPath_3)" customFrame="url(#clipPath_3)">
    ...
    @if($showAddModal)
        <div class="_flex-display _justify-content-center _align-center screen">
            <div class="window add_info_window">
                ...
                <div class="search_field search_field_input add_service_nobg">
                    <select wire:model="service_id" class="add_name_service">
                        <option value="">{{__('Оберіть послугу')}}</option>
                        @foreach($allServices as $s)
                            <option value="{{ $s['id'] }}">{{ $s['name'] }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="_flex-display _justify-content-between _align-center add_service_bottom">
                    <div class="search_field search_field_for">
                        <select wire:model="prefix" id="search_for" class="search_for">
                            <option value="for">{{__('Від')}}</option>
                            <option value="equals">=</option>
                        </select>
                    </div>
                    <div class="search_field search_field_input add_service_nobg add_price_block">
                        <input wire:model="price" id="add_price" class="add_price" type="number" placeholder="{{__('Ціна')}}" />
                    </div>
                </div>
                <button wire:click="addServiceToDoctor" class="btn rose_btn submit_window">{{__('Зберегти')}}</button>
            </div>
        </div>
    @endif
    @if($showCreateModal)
        <div class="_flex-display _justify-content-center _align-center screen">
            <div class="window add_info_window">
                <div class="_flex-display _justify-content-between _align-center window_top">
                    <h4>{{__('Створити нову послугу')}}</h4>
                    <button wire:click="$set('showCreateModal', false)" id="window_close" class="window_close">
                        <svg viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="48.000000" height="48.000000" fill="none" clip-path="url(#clipPath_6)" customFrame="url(#clipPath_6)">
                            <defs>
                                <clipPath id="clipPath_6">
                                    <rect width="48.000000" height="48.000000" x="0.000000" y="0.000000" rx="24.000000" fill="rgb(255,255,255)" />
                ...
                <div class="search_field search_field_input add_service_nobg">
                    <input type="text" wire:model="name" placeholder="{{__('Назва послуги')}}" class="add_name_service">
                </div>
                <div class="_flex-display _justify-content-between _align-center add_service_bottom">
                    <div class="search_field search_field_for">
                        <select wire:model="prefix" id="search_for" class="search_for">
                ...
                <button wire:click="createServiceAndAttach" class="btn rose_btn submit_window">{{__('Зберегти')}}</button>
            </div>
        </div>
    @endif
    @if($showEditModal)
        <div class="_flex-display _justify-content-center _align-center screen">
            <div class="window add_info_window">
                <div class="_flex-display _justify-content-between _align-center window_top">
                    <h4>{{__('Редагувати послугу')}}</h4>
                    <button wire:click="$set('showEditModal', false)" id="window_close" class="window_close">
                        <svg viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="48.000000" height="48.000000" fill="none" clip-path="url(#clipPath_6)" customFrame="url(#clipPath_6)">
                            <defs>
                                <clipPath id="clipPath_6">
                                    <rect width="48.000000" height="48.000000" x="0.000000" y="0.000000" rx="24.000000" fill="rgb(255,255,255)" />
                ...
                <div class="search_field search_field_input add_service_nobg">
                    <input type="text" wire:model="name" class="add_name_service">
                </div>
                <div class="_flex-display _justify-content-between _align-center add_service_bottom">
                    <div class="search_field search_field_for">
                        <select wire:model="prefix" id="search_for" class="search_for">
                ...
                <button wire:click="updateService" class="btn rose_btn submit_window">{{__('Зберегти')}}</button>
            </div>
        </div>
    @endif
</div>
