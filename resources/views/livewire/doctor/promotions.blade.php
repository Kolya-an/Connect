<div class="spec_register_wrapper">
    ...
    @endif
    <h5>{{__('Ваші Акції та знижки')}}</h5>
    <button wire:click="openAddModal" class="_flex-display _justify-content-center _align-center btn white_rose_btn add_action add_action_btn">
    ...
    <div class="_flex-display _justify-content-between _align-stretch action_list">
        @forelse($promotions as $promo)
            <div class="_flex-display _justify-content-between _align-center spec_action">
                <div class="spec_action_content">
                    <div class="_flex-display _justify-content-between _align-center spec_action_top">
                        <div class="btn white_rose_btn spec_action_date">Діє до {{ $promo->date_to ? $promo->date_to->format('d.m.Y') : '-' }}</div>
                        <div class="_flex-display _align-center spec_action_buttons">
                            <button wire:click="openEditModal({{ $promo->id }})" class="spec_action_edit_btn">{{__('Редагувати')}}</button>
                            <button wire:click="delete({{ $promo->id }})" class="spec_action_close_btn">
                                <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24.000000" height="24.000000" fill="none" clip-path="url(#clipPath_4)" customFrame="url(#clipPath_4)">
                                    <defs>
                                    ...
                                    <rect id="Frame 1153" width="24.000000" height="24.000000" x="0.000000" y="0.000000" rx="12.000000" fill="rgb(255,255,255)" />
                                    <g id="material-symbols:close-rounded" clip-path="url(#clipPath_5)" customFrame="url(#clipPath_5)">
                    ...
                    <div class="_flex-display _justify-content-between _align-center spec_action_bottom">
                        <div class="spec_action_bottom_content">
                            <h5>{{ $promo->title }}</h5>
        ...
        @empty
            <p>{{__('Акцій поки що немає')}}</p>
        @endforelse
    </div>
    @if($showModal)
        <div id="add_action" class="_flex-display _justify-content-center _align-center screen">
            <div class="window add_info_window" wire:key="promo-form">
                <div class="_flex-display _justify-content-between _align-center window_top">
                    <h4>{{ $editingId ? 'Редагувати акцію' : 'Додати акцію' }}</h4>
                    <button wire:click="$set('showModal', false)" id="window_close" class="window_close">
                ...
                <form wire:submit.prevent="save" id="form_action">
                    <div class="_flex-display _justify-content-between _align-center">
                        <div class="search_field search_field_input add_service_nobg" style="width: 49%">
                            <input wire:model="date_from" type="date" style="display: block;">
                        </div>
                        <div class="search_field search_field_input add_service_nobg" style="width: 49%">
                            <input wire:model="date_to" type="date" style="display: block;">
                        </div>
                    </div>
                    <div class="search_field search_field_input add_service_nobg" style="max-width: 100%;width: 100%">
                        <input wire:model.defer="title" id="add_action_title" style="max-width: 100%;width: 100%" type="text" placeholder="{{__('Назва послуги')}}">
                        @error('title') <span class="error">{{ $message }}</span> @enderror
                    </div>
                    <div class="search_field search_field_input add_service_nobg" style="max-width: 100%;width: 100%">
                        <input wire:model.defer="description" id="add_action_desc" style="max-width: 100%;width: 100%" type="text" placeholder="{{__('Опис послуги')}}">
                        @error('description') <span class="error">{{ $message }}</span> @enderror
                    </div>
                    <div class="search_field search_field_input add_service_nobg" style="max-width: 100%;width: 100%">
                        <input wire:model.defer="old_price" id="add_action_old_cost" style="max-width: 100%;width: 100%" type="number" placeholder="{{__('Вартість до знижки')}}">
                        @error('old_price') <span class="error">{{ $message }}</span> @enderror
                    </div>
                    <div class="search_field search_field_input add_service_nobg" style="max-width: 100%;width: 100%">
                        <input wire:model.defer="new_price" id="add_action_new_cost" style="max-width: 100%;width: 100%" type="number" placeholder="{{__('Вартість зі зніжкою')}}">
                        @error('new_price') <span class="error">{{ $message }}</span> @enderror
                    </div>
                    <button type="submit" class="btn rose_btn submit_window">{{__('Зберегти')}}</button>
                </form>
            </div>
        </div>
    @endif
