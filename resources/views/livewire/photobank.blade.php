        ...
        <div class="container">
            <div class="search_banner">
                <div class="search_banner_title">
                    <h3>{{__('Ми зібрали фото робіт майстрів в одному місці.')}} <span>{{__('Обирай та записуйся!')}}</span></h3>
                ...
                <div class="home_search_bg _minwidth769"><img src="{{ asset('images/photo_banner.png') }}" alt=""></div>
            </div>
    ...
    <div id="photo_list">
        <div class="container">
            ...
            @else
                ...
                <!-- Пагинация -->
                @if($paginatedPhotos->hasPages())
                    <ul class="_flex-display _justify-content-center _align-center pagination">
                        <!-- Первая страница -->
                        <li>
                            <a class="_flex-display _justify-content-center _align-center"
                               wire:click="setPage(1)"
                               wire:loading.attr="disabled"
                               @if($paginatedPhotos->onFirstPage()) style="opacity: 0.5; cursor: not-allowed;" @endif>
                        ...
                        @php
                            ...
                            $lastPage = $paginatedPhotos->lastPage();
                            $startPage = max(1, $currentPage - 2);
                            $endPage = min($lastPage, $currentPage + 2);
                        @endphp

                        @if($startPage > 1)
                            <li><a class="_flex-display _justify-content-center _align-center" href="#" wire:click="setPage(1)">1</a></li>
                            @if($startPage > 2)
                        ...
                        @for($page = $startPage; $page <= $endPage; $page++)
                            <li class="{{ $page == $currentPage ? 'pagination_current' : '' }}">
                                <a class="_flex-display _justify-content-center _align-center"
                        ...
                        @if($endPage < $lastPage)
                            @if($endPage < $lastPage - 1)
                                <li class="pagination_dots"><a class="_flex-display _justify-content-center _align-center">...</a></li>
                            @endif
                            <li><a class="_flex-display _justify-content-center _align-center" href="#" wire:click="setPage({{ $lastPage }})">{{ $lastPage }}</a></li>
                        @endif
                        ...
                        <li>
                            <a class="_flex-display _justify-content-center _align-center"
                               wire:click="nextPage"
                               wire:loading.attr="disabled"
                               @if(!$paginatedPhotos->hasMorePages()) style="opacity: 0.5; cursor: not-allowed;" @endif>
                                <svg viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16.000000" height="16.000000" fill="none">
                        ...
                        <li>
                            <a class="_flex-display _justify-content-center _align-center"
                               wire:click="setPage({{ $paginatedPhotos->lastPage() }})"
                               wire:loading.attr="disabled"
                               @if(!$paginatedPhotos->hasMorePages()) style="opacity: 0.5; cursor: not-allowed;" @endif>
                                <svg viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16.000000" height="16.000000" fill="none">
