                ...
                <div class="_flex-display _justify-content-between spec_review_top">
                    <div class="_flex-display _align-center spec_review_left">
                        ...
                        <div class="spec_review_name_stars">
                            ...
                            @endif
                            <p class="client_address _minwidth769"><span>{{__('Адреса')}}:</span> {{ $appointment->doctor->city }},  {{ $appointment->doctor->address }}</p>
                        </div>
                    </div>
                    <p class="client_address _maxwidth768"><span>{{__('Адреса')}}:</span> {{ $appointment->doctor->city }},  {{ $appointment->doctor->address }}</p>

                    <a wire:click.prevent="showModal({{ $appointment->id }})" class="white_rose_btn register_prev _maxwidth768">{{__('Скасувати візит')}}</a>
                    ...
                    <div class="spec_review_date _minwidth769">
                        ...
                        <a wire:click.prevent="showModal({{ $appointment->id }})" class="white_rose_btn register_prev">{{__('Скасувати візит')}}</a>
                    </div>
                </div>
            </div>
        @endforeach
                    ...
                    <form wire:submit.prevent="cancelAppointment">
                        <label class="custom-radio">
                            <input type="radio"
                                   ...
                                   wire:click="setReason('Запис зроблено випадково')"
                                   value="Запис зроблено випадково">
                            <span></span> {{__('Запис зроблено випадково')}}
                        </label>
                        <label class="custom-radio">
                            <input type="radio"
                                   wire:model="selectedReason"
                                   value="Змінились плани"
                                   wire:click="setReason('Змінились плани')">
                            <span></span> {{__('Змінились плани')}}
                        </label>
                        <label class="custom-radio">
                            <input type="radio"
                                   wire:model="selectedReason"
                                   value="Перенесла запис на інший день"
                                   wire:click="setReason('Перенесла запис на інший день')">
                            <span></span> {{__('Перенесла запис на інший день')}}
                        </label>
