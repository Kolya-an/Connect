...
class Socialite1Controller extends Controller
{
    /**
    ...
    public function handleProviderCallback(string $provider)
    {
        ...
        try {
            ...
            $userExists = User::where('email', $socialiteUser->getEmail())->exists();
            if (!$userExists) {
                $redirectUrl = session('redirect_url', '/');

                // Можно также сохранить данные социального пользователя в сессии
                // для возможности ручной регистрации
                session([
                    'social_user' => [
                ...
                return redirect($redirectUrl)
                    ->with('warning', 'Аккаунт не найден. Вы можете зарегистрироваться.');
            }
        } catch (\Exception $e) {
